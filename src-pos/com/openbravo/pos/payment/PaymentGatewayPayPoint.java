...
public class PaymentGatewayPayPoint implements PaymentGateway {
    ...
    public PaymentGatewayPayPoint(AppProperties props) {
        ...
        // SSL is now built-in to modern Java versions (7+), no need for deprecated
        // provider
        ...
        m_sCurrency = (Locale.getDefault().getCountry().isEmpty()) ? Currency.getInstance("EUR").getCurrencyCode()
    ...
    public void execute(PaymentInfoMagcard payinfo) {
        ...
        try {
            ...
            if (payinfo.getTotal() > 0.0) {
                ...
                methodParams = new Object[] { m_sCommerceID, m_sCommercePassword, payinfo.getTransactionID(), // unique
                        InetAddress.getLocalHost().getHostAddress(), payinfo.getHolderName(), payinfo.getCardNumber(),
                        Double.toString(payinfo.getTotal()), payinfo.getExpirationDate(), "", // issue number
                        "", // start date
                        "", // order
                        "", // shipping
                        "", // billing
                        // Options (StringUtils.getCardNumber())
                        (m_bTestMode ? "test_status=true," : "test_status=live,") + "dups=false,currency="
                                + m_sCurrency };
            } else {
                // payinfo.paymentError(AppLocal.getIntString("message.paymentrefundsnotsupported"));
                call.setOperationName(OPERATIONREFUND);
                methodParams = new Object[] { m_sCommerceID, m_sCommercePassword, payinfo.getTransactionID(),
                        Double.toString(Math.abs(payinfo.getTotal())), "secpay",
                        "refund" + payinfo.getTransactionID() };
            }

            String returned = (String) call.invoke(methodParams);
            // "?valid=true&trans_id=Twem0003&code=A&auth_code=9999&message=TEST
            // AUTH&currency=EUR&amount=50.0&test_status=true"
            ...
            if (returned == null) {
                ...
                if ("true".equals(props.get("valid"))) {
                    ...
                    if ("N".equals(sCode)) {
                        ...
                        // P:T Pre-bank checks. Card type invalid - i.e. does not match card number
                        // prefix
                        payinfo.paymentError(AppLocal.getIntString("message.paymenterror"),
                        ...
                        // P:R Pre-bank checks. Transaction timed out awaiting a virtual circuit.
